//
//  BadgeViewModelDeprecatedTests.swift
//  SparkComponentBadge
//
...
final class BadgeViewModelDeprecatedTests: XCTestCase {
    ...
    func test_init() throws {
        for badgeIntent in BadgeIntentType.allCases {
            ...
            let viewModel = BadgeViewModelDeprecated(theme: theme, intent: badgeIntent)

            let badgeExpectedColors = BadgeGetIntentColorsUseCaseDeprecated().execute(intentType: badgeIntent, on: theme.colors)
    ...
    func test_set_value() throws {
        for badgeIntent in BadgeIntentType.allCases {
            ...
            let viewModel = BadgeViewModelDeprecated(theme: theme, intent: badgeIntent, value: 20)
    ...
    func test_theme_change_publishes_values() {
        // Given
        let sut = BadgeViewModelDeprecated(theme: self.theme, intent: .danger)
    ...
    func test_size_change_publishes_values() {
        // Given
        let sut = BadgeViewModelDeprecated(theme: self.theme, intent: .danger, size: .medium)
        let updateExpectation = expectation(description: "Attributes updated")
        updateExpectation.expectedFulfillmentCount = 2
    ...
    func test_value_change_publishes_values() {
        // Given
        let sut = BadgeViewModelDeprecated(theme: self.theme, intent: .danger, size: .medium, value: 9)
        let updateExpectation = expectation(description: "Attributes updated")
        updateExpectation.expectedFulfillmentCount = 2
    ...
    func test_formater_change_publishes_values() {
        // Given
        let sut = BadgeViewModelDeprecated(theme: self.theme, intent: .danger, value: 9999, format: .default)
        let updateExpectation = expectation(description: "Attributes updated")
        updateExpectation.expectedFulfillmentCount = 2
...
private extension BadgeBorderDeprecated {
